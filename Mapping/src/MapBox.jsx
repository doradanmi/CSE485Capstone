...
function MapBox({ flights, time }) {
  ...
  useEffect(() => {
    ...
    const newDronePositions = flights.map((flight) => {
      ...
      const distance = getDistance(start.lat, start.long, end.lat, end.long);
  ...
  return (
    ...
    >
      ...
      {dronePositions.map((dronePosition, index) => (
        <div key={index}>
          <Marker latitude={dronePosition.lat} longitude={dronePosition.long}>
            <img
              className="w-6"
              src={drone}
              style={{ transform: `rotate(${dronePosition.rotation}deg)` }}
            />
          </Marker>
          <Source
            id={`geofence-${index}`}
            ...
            data={{
              ...
              geometry: {
                type: "Point",
                coordinates: [dronePosition.long, dronePosition.lat],
              },
              properties: {
                radius: 3000,
              },
